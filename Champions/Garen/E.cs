...
namespace Spells
{
    public class GarenE : GameScript
    {
        ...
        {
            var p = ApiFunctionManager.AddParticleTarget(owner, "Garen_Base_E_Spin.troy", owner);
            var visualBuff = ApiFunctionManager.AddBuffHUDVisual("GarenE", 3.0f, 1, BuffType.Aura, owner);
        ...
        private void ApplySpinDamage(Champion owner, Spell spell, AttackableUnit target)
        {
            ...
            foreach (var unit in units)
            {
                if (unit.Team != owner.Team)
                {
                    ...
                    var ad = new[] {.7f, .8f, .9f, 1f, 1.1f}[spell.Level - 1] * owner.Stats.TotalAttackDamage *
                               0.5f;
